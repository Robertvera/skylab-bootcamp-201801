    ...
    function printSearch(artist) {
        ...
        artist.forEach(function (artist) {            
            if (artist.images.length < 1) {                
                $('#artist-list').append(`<a href="#" id="artist-card" data-id="${artist.id}"><div class="card" id="artist-card-padding"><div class="col card-image image-hover img-zoom-in"><img src="https://s.discogs.com/images/default-artist.png"><span class="card-title" id="artist-name">${artist.name}</span></div></div></a>`)
            } else {                
                $('#artist-list').append(`<a href="#" id="artist-card" data-id="${artist.id}"><div class="card" id="artist-card-padding"><div class="col card-image image-hover img-zoom-in"><img src="${artist.images[0].url}" "><span class="card-title" id="artist-name">${artist.name}</span></div></div></a>`)
            }                                  
        })
    }
